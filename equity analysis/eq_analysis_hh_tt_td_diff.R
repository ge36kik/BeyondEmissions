...
hh_all <- hh_metrics %>%
  left_join(hh_mode_share, by = c("hhid", "oek_status_gr"))

# Aggregate to oek_status_gr level 
hh_oek_status_metrics <- hh_all %>%
...
### Scenario 1 
# hh-level metrics
legs1_week_hh <- legs1_joined %>%
  filter(person_id %in% legs_week_hh$person_id) %>%
  filter(day != 6) %>%
  filter(day != 7)

#consistency check
# filter base scenario so that in both scenarios are the same persons
length(unique(legs_week_hh$hhid))
length(unique(legs1_week_hh$hhid))
...
legs_week_hh <- legs_week_hh %>%
  semi_join(legs1_week_hh, by = "hhid")

hh1_metrics <- legs1_week_hh %>%
  group_by(hhid, oek_status_gr) %>%
  summarise(
    mean_time_min = mean(time_min, na.rm = TRUE),
    mean_distance_m = mean(distance_m, na.rm = TRUE),
...
# Combine hh-level metrics + mode shares
hh1_all <- hh1_metrics %>%
  left_join(hh1_mode_share, by = c("hhid", "oek_status_gr"))

# Aggregate to oek_status_gr level 
hh_oek_status1_metrics <- hh1_all %>%
  group_by(oek_status_gr) %>%
  summarise(
...
hh_metrics_diff <- hh_oek_status_metrics %>%
  inner_join(hh_oek_status1_metrics, by = "oek_status_gr") %>%
