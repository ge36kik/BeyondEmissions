...
# read in file with Munich's target shares from 2022
...
# ---------- Diesel choice calibration for multiple car households -------------
...
# filter household set to households only owning multiple cars without Munich
hh_synpop_10pct_2CHH <- hh_synpop_10pct %>%
  filter(JUR_NAME != 'Muenchen Landeshauptstadt') %>%
  filter(H_ANZAUTO > 1)
# distribution 1CHH-MCHH: 70%-30%
# distribution 2CHH-3CHH: 80%-20%

# filter household set to households only owning multiple cars in Munich
hh_synpop_10pct_2CHH_MUC <- hh_synpop_10pct %>%
  filter(JUR_NAME == 'Muenchen Landeshauptstadt') %>%
  filter(H_ANZAUTO > 1)

# when predicting with the model, NAs were generated --> eliminate them from the input data
hh_synpop_10pct_2CHH <- na.omit(hh_synpop_10pct_2CHH)
hh_synpop_10pct_2CHH_MUC <- na.omit(hh_synpop_10pct_2CHH_MUC)
...
hh_synpop_10pct_2CHH_MUC <- hh_synpop_10pct_2CHH_MUC %>%
  left_join(
...
hh_synpop_10pct_2CHH_MUC <- hh_synpop_10pct_2CHH_MUC %>% rename(JUR_NAME_MUC = JUR_NAME)
hh_synpop_10pct_2CHH_MUC <- hh_synpop_10pct_2CHH_MUC %>% rename(JUR_NAME = geo_name)

# combine both data sets to have all Landkreise and Munichs districts in one data file
common_cols <- intersect(names(hh_synpop_10pct_2CHH), names(hh_synpop_10pct_2CHH_MUC))
merged_df <- merge(hh_synpop_10pct_2CHH, hh_synpop_10pct_2CHH_MUC, by = common_cols, all = TRUE)
test <- hh_synpop_10pct %>%
  filter(H_ANZAUTO > 1)
#check is passed: sum of observations in the merged data file equals the original data file
hh_synpop_10pct_2CHH <- merged_df
...
hh_synpop_10pct_2CHH$JUR_NAME_MUC %>% replace_na("nonMUC")
...
hh_synpop_10pct_2CHH <- hh_synpop_10pct_2CHH %>%
  mutate(
    base_util = if_else(
      car_rank == 1,
      predict(`glm_Diesel_1st_2-5CHH_implementation_final_gender_withoutmileage`, newdata = ., type = "link"),
      predict(`glm_Diesel_2nd_2-5CHH_implementation_final_gender_withoutmileage`, newdata = ., type = "link")
    )
  )
...
for (s in seeds) {
  ...
  prob_mat <- matrix(rep(hh_synpop_10pct_2CHH$base_util, each = 1), ncol = 1)
  # Convert utility to probability (logit)
  prob_vec <- 1 / (1 + exp(-hh_synpop_10pct_2CHH$base_util))
  
  # Probabilistic diesel choices
  diesel_choice <- rbinom(nrow(hh_synpop_10pct_2CHH), 1, prob_vec)
  
  decision_list[[s]] <- hh_synpop_10pct_2CHH %>%
    mutate(diesel_choice = diesel_choice)#,
           #mean_choice = diesel_choice)  # initial mean_choice = diesel_choice
}
...
print(seed_scores)
# best seed is 1 with max 0.0273

# update the dataframe to continue with the emission class assignment
hh_synpop_10pct_2CHH1 <- as.data.frame(calibrated_list[1])
...
hh_synpop_10pct_2CHH1 <- read.csv("C://Users//tumme//Documents//calibrated_Simulation_Data_from_R//calibration_output_2CHH_20250816//hh_synpop_10pct_2CHH_diesel_all_seeds_calibrated_20250816.csv")
#res <- read.csv("C://Users//tumme//Documents//mlogitfirst_20250829.csv", sep=";")
#res2 <- read.csv("C://Users//tumme//Documents//mlogitsecond_20250829.csv", sep=";")

# get all diesel cars
hh_synpop_10pct_2CHHD <- hh_synpop_10pct_2CHH1 %>%
  filter(seed == 1) %>%
...
# Prepare long format for predictions
hh_synpop_long <- hh_synpop_10pct_2CHHD %>%
  select(id, JUR_NAME, car_rank, row_xd, everything()) %>%
...
### compute utilities and probabilities (functions are in another script)
...
# rearrange the data sets to a wide format
df_first <- res %>%
...
# combine utilites and probabilities of the first and the other cars to one data frame
base_probs_df <- bind_rows(df_first, df_other)

# Join utilities and probabilities to the synpop
hh_synpop_10pct_2CHHD <- hh_synpop_10pct_2CHHD %>%
  left_join(base_probs_df, by = "row_xd")

hh_synpop_10pct_2CHHD <- hh_synpop_10pct_2CHHD %>% rename(base_EUR4 = utility_EUR4) 
hh_synpop_10pct_2CHHD <- hh_synpop_10pct_2CHHD %>% rename(base_EUR5 = utility_EUR5) 
hh_synpop_10pct_2CHHD <- hh_synpop_10pct_2CHHD %>% rename(base_EUR6 = utility_EUR6)

# --- Step 1: Create 10 seed-specific data sets ---
seeds <- 1:10
...
for (s in seq_along(seeds)) {
  ...
  eur_datasets[[s]] <- hh_synpop_10pct_2CHHD %>%
    mutate(
      util_EUR4 = base_EUR4,
      util_EUR5 = base_EUR5,
      util_EUR6 = base_EUR6
...
for (s in seq_along(seeds)) {
  ...
  # old, but also correct verion for probabilistic assignment with the R internal function sample()
  #hh_seed <- hh_seed %>%
  #  rowwise() %>%
  #  mutate(
  ...
  hh_seed <- hh_seed %>%
    rowwise() %>%
    mutate(
      u = runif(1),  #uniform distribution of the random number
      EUR_choice_updated = case_when(
        u < p_EUR4 ~ "EUR4",
        u < p_EUR4 + p_EUR5 ~ "EUR5",
        TRUE ~ "EUR6"
      )
    ) %>%
    ungroup()
...
# --- select "best seed" according to min difference between realized and target share
# Summarize realized shares per seed and region
...
ggplot(eur_variance, aes(y = range_EUR6)) +
  geom_boxplot(fill = "lightblue", alpha = 0.6) +
  labs(title = "Distribution of EURO_6 share ranges across regions and seeds",
       y = "range") +
  theme_minimal()
...
hh_synpop_10pct_2CHHnD <- hh_synpop_10pct_2CHH1 %>%
  filter(seed == 1) %>%
  filter(diesel_choice_final == 0)

common_cols <- intersect(names(hh_synpop_10pct_2CHHD1), names(hh_synpop_10pct_2CHHnD))
merged_2CHHDnD <- merge(hh_synpop_10pct_2CHHD1, hh_synpop_10pct_2CHHnD, by = common_cols, all = TRUE)
merged_2CHHDnD <- merged_2CHHDnD %>% rename(emissionClass = EUR_choice_updated)
merged_2CHHDnD <- merged_2CHHDnD %>%
  mutate(emissionClass = replace_na(emissionClass, "EUROx"))
merged_2CHHDnD$emissionClass <- factor(merged_2CHHDnD$emissionClass, 
                                       levels = c("EUR4", "EUR5", "EUR6", "EUROx"),
                                       labels = c("EURO_4", "EURO_5", "EURO_6", "EUROx"))
merged_2CHHDnD$diesel_choice <- factor(merged_2CHHDnD$diesel_choice_final, 
                                       levels = c(0,1),
                                       labels = c("nonDIESEL", "DIESEL"))


setwd("C:/Users/tumme/Desktop")
# write calibrated population and calibration parameters to csv files
write.csv(merged_2CHHDnD, file = "hh_synpop_10pct_2CHH_20250830.csv", row.names = FALSE)

write.csv(calib_long, file = "hh_synpop_10pct_2CHH_diesel_all_seeds_calibrated_20250816.csv", row.names = FALSE)
write.csv(lkr_constants_diesel, file = "Diesel_constants_2CHH_20250816.csv", row.names = FALSE)
write.csv(regional_diesel, file = "Diesel_2CHH_target_calibrated_distribution_20250816.csv", row.names = FALSE)
write.csv(region_scores, file = "Diesel_2CHH_target_calibrated_distribution_selectedseed6_20250816.csv", row.names = FALSE)

write.csv(calib_long2, file = "hh_synpop_10pct_2CHH_EUR_all_seeds_calibrated_20250830.csv", row.names = FALSE)
write.csv(lkr_constants_eur, file = "EUR_constants_2CHH_20250830.csv", row.names = FALSE)
write.csv(regional_eur, file = "EUR_2CHH_target_calibrated_distribution_20250830.csv", row.names = FALSE)
write.csv(eur_eval, file = "EUR_2CHH_target_calibrated_distribution_selectedseed5_20250830.csv", row.names = FALSE)
